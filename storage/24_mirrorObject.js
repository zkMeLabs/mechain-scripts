...
export const main = async () => {
  try {
    const { rpc, contracts, storageAddress, privateKey, bucketName, objectName } =
    ...
    const tx = await storage.mirrorObject(0, bucketName, objectName, dstChainId);
    const receipt = await tx.wait();
    console.log('start mirror object success, receipt: ', receipt);
  } catch (error) {
